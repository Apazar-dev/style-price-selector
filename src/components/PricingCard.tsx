      ...
      <div className={`p-8 ${isPopular ? 'pt-16' : ''}`}>
        ...
        <div className="space-y-4">
          {features.map((feature, index) => <div key={index} className="flex items-start">
              <div className={`flex-shrink-0 mt-1 ${feature.included ? isPopular ? 'text-white' : 'text-green-500' : 'text-gray-400'}`}>
                {feature.included ? <Check size={16} /> : <X size={16} />}
              </div>
              <div className={`ml-3 text-sm ${isPopular ? 'text-white' : feature.included ? 'text-gray-900' : 'text-gray-400'}`}>
                <p className={feature.included ? '' : 'line-through'}>{feature.text}</p>
                {feature.details && feature.included && <ul className={`mt-1 ml-4 list-disc space-y-1 ${isPopular ? 'text-white opacity-90' : 'text-gray-600'}`}>
