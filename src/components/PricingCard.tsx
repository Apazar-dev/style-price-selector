...
const PricingCard = ({
  ...
  return (
    <div className={`relative rounded-2xl shadow-lg overflow-hidden transition-all duration-300 ${isPopular ? 'bg-turquoise-light text-white scale-105' : 'bg-white hover:shadow-xl'} ${isSelected ? 'ring-4 ring-turquoise-light' : ''}`}>
      {isPopular && (
      ...
      <div className={`p-8 ${isPopular ? 'pt-16' : ''}`}>
        <div className="text-center mb-6">
          ...
          <div className="mb-4">
            ...
            {originalPrice && (
              <span className={`text-lg line-through ml-2 ${isPopular ? 'text-white opacity-70' : 'text-gray-400'}`}>
                {originalPrice}€
              </span>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="mt-2 text-center">
              <a
                href="https://www.illustrestudios.com/#formats"
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center gap-1 text-xs ${isPopular ? 'text-white/80 hover:text-white' : 'text-gray-500 hover:text-turquoise-dark'} transition-colors`}
              >
                Voir nos formats
            ...
            <div key={index} className="flex items-start">
              ...
              <div className={`ml-3 text-sm ${isPopular ? 'text-white' : 'text-gray-900'}`}>
                ...
                {feature.details && feature.status === 'included' && (
                  <ul className={`mt-1 ml-4 list-disc space-y-1 ${isPopular ? 'text-white opacity-90' : 'text-gray-600'}`}>
