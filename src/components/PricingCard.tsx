...
const PricingCard = ({ 
  ...
  return (
    <div className={`relative bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 ${
      isPopular ? 'bg-turquoise-light text-white scale-105' : 'hover:shadow-xl'
      ...
      <div className={`p-8 ${isPopular ? 'pt-16' : ''}`}>
        <div className="text-center mb-6">
          ...
          <p className={`text-sm mb-4 ${isPopular ? 'text-white/90' : 'text-gray-600'}`}>
            {subtitle}
          </p>
          ...
          <div className="mb-4">
            ...
            {originalPrice && (
              <span className={`text-2xl line-through ml-2 ${isPopular ? 'text-white/70' : 'text-gray-400'}`}>
                {originalPrice}€
              </span>
            )}
          </div>
          ...
          <div className="mb-6">
            <Select onValueChange={onFormatChange} value={selectedFormat}>
              <SelectTrigger className={`w-full ${
                isPopular 
                  ? 'bg-turquoise-dark border-turquoise-dark text-white' 
          ...
          <button
            ...
            className={`w-full py-3 px-6 rounded-full font-medium transition-all duration-300 ${
              isSelected
                ...
                : isPopular 
                  ? 'bg-turquoise-dark text-white hover:bg-opacity-90' 
        ...
        <div className="space-y-4">
          {features.map((feature, index) => (
            <div key={index} className="flex items-start">
              <div className={`flex-shrink-0 mt-1 ${feature.included ? 'text-green-500' : 'text-gray-400'}`}>
                {feature.included ? <Check size={16} /> : <X size={16} />}
              </div>
              <div className={`ml-3 text-sm ${
                isPopular ? 'text-white' : feature.included ? 'text-gray-900' : 'text-gray-400'
              }`}>
                <p className={feature.included ? '' : 'line-through'}>{feature.text}</p>
                {feature.details && feature.included && (
                  <ul className={`mt-1 ml-4 list-disc space-y-1 ${isPopular ? 'text-white/90' : 'text-gray-600'}`}>
